...
cd "$BACKEND_DIR" || { echo "ERROR: No existe $BACKEND_DIR" >> "$OUTPUT_FILE"; exit 1; }
...
echo "--- Schema de Prisma (primeras 30 líneas) ---" >> "$OUTPUT_FILE"
head -30 prisma/schema.prisma >> "$OUTPUT_FILE" 2>&1 || echo "No se encontró schema" >> "$OUTPUT_FILE"
echo >> "$OUTPUT_FILE"

echo "--- Conteo de registros por tabla ---" >> "$OUTPUT_FILE"
npx prisma db execute --stdin <<SQL 2>&1 | grep -v "Prisma" | grep -v "^$" | head -20 >> "$OUTPUT_FILE" || echo "Error al contar registros" >> "$OUTPUT_FILE"
SELECT 
...
cd "$FRONTEND_DIR" || { echo "ERROR: No existe $FRONTEND_DIR" >> "$OUTPUT_FILE"; exit 1; }
